...
class TestdataUtils():
    ...
    def def_testcase_data_list_by_mysql(self, case_step_id=None):
        """从MySQL获取测试用例数据"""
        if case_step_id:
            case_data = self.sql_utils.get_mysql_test_single_case_info(case_step_id)
            return ({'case_id': case_step_id, 'case_info': case_data},)
    ...
    def get_single_testcase(self, case_step_id):
        """获取单条测试用例数据"""
        logger.info(f"开始获取单条用例数据, case_step_id={case_step_id}")
        try:
            case_data = self.def_testcase_data_list_by_mysql(case_step_id)[0]
            logger.debug(f"获取到用例数据: {case_data}")
            return {
                'case_id': case_step_id,
                'case_info': case_data['case_info']
            }
        except IndexError:
            logger.error(f"未找到对应测试用例: {case_step_id}")
            raise ValueError(f"用例数据不存在: {case_step_id}")
        except Exception as e:
            logger.error(f"获取用例数据异常: {str(e)}", exc_info=True)
            raise
