...
class ExcelToMysqlImporter:
    def __init__(self):
        ...
        # 修正字段映射：确保"模块名称"对应"part_name"
        self.field_mapping = {
            "测试用例编号": [("case_info", "case_id"), ("case_step_info", "case_id")],
            "模块名称": [("case_step_info", "part_name")],  # 关键：Excel"模块名称"→表"part_name"
    ...
    def _get_api_id_by_name(self, api_name):
        """按api_name查询api_id（api_info唯一键：api_name）"""
    ...
    def _get_case_info_id(self, case_id, case_name):
        """按case_id+case_name查询case_info.id（case_info唯一键）"""
        sql = """SELECT id FROM case_info WHERE case_id = %s AND case_name = %s LIMIT 1"""
        result = self.sql_utils.execute_query(sql, (case_id, case_name))
        return result[0]["id"] if result else None

    def _get_case_step_id(self, case_id, part_name):
        """按case_id+part_name查询case_step_info.id（case_step_info唯一键）"""
        sql = """SELECT id FROM case_step_info WHERE case_id = %s AND part_name = %s LIMIT 1"""
        result = self.sql_utils.execute_query(sql, (case_id, part_name))
        return result[0]["id"] if result else None

    def _handle_api_info(self, excel_row):
        """处理api_info：按api_name增量同步"""
    ...
    def _handle_case_info(self, excel_row):
        """处理case_info：按case_id+case_name增量同步，返回case_info.id（供case_step_info复用）"""
        ...
        if exist_case_id:
            ...
            insert_sql = """
                INSERT INTO case_info (id, case_id, case_name, is_run)
                VALUES (%s, %s, %s, %s)
            """
            self.sql_utils.execute_update(insert_sql, (new_case_id, case_id, case_name, is_run))
            logger.debug("【插入case_info】case_id=%s，case_name=%s，新id=%s", case_id, case_name, new_case_id)
            return new_case_id

    def _handle_case_step_info(self, excel_row, case_info_id, api_id):
        """处理case_step_info：id复用case_info.id（外键关联），按case_id+part_name增量同步"""
        # 1. 提取必要字段（为空时设默认值，避免非空约束报错）
        case_id = excel_row.get("测试用例编号", "").strip()
        part_name = excel_row.get("模块名称", "").strip() or "未分类"  # 模块名空时设默认值
        ...
        if exist_step_id:
            ...
            # 插入新步骤（id=case_info.id，满足外键约束）
            insert_sql = """
                INSERT INTO case_step_info (
                    id, case_id, case_step_name, part_name, api_id,
            ...
            self.sql_utils.execute_update(
                insert_sql,
                (case_info_id, case_id, case_step_name, part_name, api_id,  # id=case_info_id
                 get_value_type, variable_name, excepted_result_type,
                 excepted_result, get_value_code, is_pass)
            )
            logger.debug("【插入case_step_info】case_id=%s，part_name=%s，step_id=%s（复用case_info.id）",
            ...
            try:
                ...
                for excel_row in case_info_rows:
                    with self.sql_utils.transaction():
                        # 步骤1：处理api_info，获取api_id
                        api_id = self._handle_api_info(excel_row)
                        if not api_id:
                            raise Exception("api_info处理失败，未获取到api_id")

                        # 步骤2：处理case_info，获取case_info.id（供step复用）
                        case_info_id = self._handle_case_info(excel_row)
                        if not case_info_id:
                            raise Exception("case_info处理失败，未获取到case_info.id")

                        # 步骤3：处理case_step_info（复用case_info.id，满足外键约束）
